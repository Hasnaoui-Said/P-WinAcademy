...
CREATE TABLE IF NOT EXISTS college (
    ...
    site VARCHAR(50),
    id_adresse INT ,
	id_adresse INT REFERENCES adresse
);
...
CREATE TABLE IF NOT EXISTS role (
    name VARCHAR (30) SERIAL PRIMARY KEY,
    description VARCHAR(255),
);

CREATE TABLE IF NOT EXISTS user_role (
    LIKE users INCLUDING INDEXES,
    LIKE role INCLUDING INDEXES,
    date DATE
);
...
CREATE TABLE IF NOT EXISTS departement (
    ...
    name VARCHAR(100),
    description VARCHAR (255),
	id_college INT REFERENCES colleges ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS enseignant (
	LIKE users INCLUDING INDEXES
	id_matiere INT REFERENCES matiere ON DELETE CASCADE,
	id_departement INT REFERENCES departement ON DELETE CASCADE,
    dateFonction Date
)inherits(users);
...
CREATE TABLE IF NOT EXISTS responsable (
    dateDebut DATE,
    dateFin DATE,
    id_enseignant REFERENCES enseignant(id) ON DELETE CASCADE,
    id_departement REFERENCES departement ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS evaluation (
    date DATE,
    note DOUBLE,
    id_etudiant REFERENCES etudiant ON DELETE CASCADE,
    id_matiere REFERENCES matiere ON DELETE CASCADE
);
